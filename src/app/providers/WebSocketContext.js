...
import {addImage, setCurrentImageSelected, updateImage} from '../store/slices/imageSlice';
...
export const WebSocketProvider = ({ children }) => {
                ...
                try {
                    ...
                    if (metaData.action === "normal_quality_append") {
                        ...
                        if (imagesSelectorRef.current.hasOwnProperty(id)) {
                            ...
                            dispatch(updateImage({ id, newImageData: {
                        ...
                        if (imagesSelectorRef.current.hasOwnProperty(mediaId)) {
                            dispatch(updateImage({ id: mediaId, newImageData: {
                                ...update
                            }}));
                        }
                        return;
                    ...
                    const normalizeImage = (photo) => ({
                        ...photo,
                        posted_at: photo.posted_at
                            ? new Date(photo.posted_at).toISOString()
                            : null
                    });
                    ...
                    if (Array.isArray(metaData.media) && metaData.media.length) {
                        for (const photo of metaData.media) {
                            ...
                            mediaUrl = "https://api.doyoula.com/media/" + photo.id;

                            const normalized = normalizeImage(photo);

                            const imageData = {
                                ...normalized,
                                media_url: mediaUrl,
                                fullUrl: mediaUrl + "/full"
                            };

                            if (!imagesSelectorRef.current.hasOwnProperty(id)) {
                                dispatch(addImage({ id, imageData }));
                            }
                            dispatch(updateImage({ id, newImageData: imageData }));
    ...
    const addHandler = useCallback((action, handler) => {
        handlersRef.current[action] = handler;
    }, []);

    const deleteHandler = useCallback((action) => {
